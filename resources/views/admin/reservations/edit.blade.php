                ...
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.reservations.update', $reservation->id) }}">
                        ...
                        <div class="sm:col-span-6 pt-5">
                            <label for="status" class="block text-sm font-medium text-gray-700">Canoe</label>
                            <div class="mt-1">
                                <select id="canoe_id" name="canoe_id" class="form-multiselect block w-full mt-1">
                                    @foreach ($canoes as $canoe)
                                        <option value="{{ $canoe->id }}" @selected($canoe->id == $reservation->canoe_id)>
                                            {{ $canoe->name }}
                                            ({{ $canoe->guest_number }} Guests)
                                        </option>
                                    @endforeach
                                </select>
                            </div>
